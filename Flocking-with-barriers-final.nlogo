...
breed [ birds bird ]
;;birds is all objects

birds-own [;;change for birds
...
to setup-birds
  create-birds population
    [ set color yellow - 2 + random 7  ;; random shades look nice
      ...
      while [ any? obstacles with [ overlap myself > 0 ] ] [ ;keep checking for overlap until none
        setxy random-xcor random-ycor]
...
to setup-obs [ obstacle-placement]

  set-default-shape obstacles "circle"
  ;;set num-obstacles num-obstacles
  create-obstacles num-obstacles [
    set color grey - 3
    ; First, set the size of the obstacle according to user input
    set size size-of-obstacle
    setxy random-xcor random-ycor
...
to go
  ...
  ;; the following line is used to make the birds
...
to find-obstacles
  set near-obs obstacles in-cone 10 90
end


to-report near-obstacles
  report any? obstacles-on patch-ahead 10
end
...
to find-flockmates  ;; bird procedure
  set flockmates other birds in-cone vision vision-angle
end

to find-nearest-neighbor ;; bird procedure
  set nearest-neighbor min-one-of flockmates [distance myself]
end

to find-nearest-obs ;; bird procedure
  set nearest-obs min-one-of near-obs [distance myself]
end

;;AVOID

to avoid
  turn-away heading-near-obs 50
end

to-report heading-near-obs  ;; bird procedure
  ;; again we want the heading from me to the obstacle,
  ;; so we add 180
  let x-component [sin (towards myself + 180)] of nearest-obs
  let y-component [cos (towards myself + 180)] of nearest-obs
...
to separate  ;; bird procedure
  turn-away ([heading] of nearest-neighbor) max-separate-turn
end

;;; ALIGN

to align  ;; bird procedure
  turn-towards average-flockmate-heading max-align-turn
end

to-report average-flockmate-heading  ;; bird procedure
...
to cohere  ;; bird procedure
  turn-towards average-heading-towards-flockmates max-cohere-turn
end

to-report average-heading-towards-flockmates  ;; bird procedure
  ;; "towards myself" gives us the heading from the other bird
  ;; to me, but we want the heading from me to the other bird,
  ;; so we add 180
...
to turn-towards [new-heading max-turn]  ;; bird procedure
  turn-at-most (subtract-headings new-heading heading) max-turn
end

to turn-away [new-heading max-turn]  ;; bird procedure
  turn-at-most (subtract-headings heading new-heading) max-turn
end

;; turn right by "turn" degrees (or left if "turn" is negative),
;; but never turn more than "max-turn" degrees
to turn-at-most [turn max-turn]  ;; bird procedure
